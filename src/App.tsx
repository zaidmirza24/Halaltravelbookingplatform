...
function App() {
  ...
  const [currentPage, setCurrentPage] = useState<'home' | 'search' | 'detail' | 'booking' | 'dashboard'>('home');
  
  // Allow switching between pages for demo purposes
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* Demo Page Switcher */}
      <div className="bg-emerald-600 text-white py-3 sticky top-20 z-40">
        <div className="container-custom">
          <div className="flex flex-wrap items-center justify-center gap-2 md:gap-4">
            <span className="text-xs md:text-sm font-medium w-full md:w-auto text-center mb-2 md:mb-0">Demo Pages:</span>
            <button 
              onClick={() => setCurrentPage('home')}
              className={`px-3 md:px-4 py-1 rounded-full text-xs md:text-sm transition-colors ${currentPage === 'home' ? 'bg-white text-emerald-600' : 'bg-white/20 hover:bg-white/30'}`}
            >
              Home
            </button>
            <button 
              onClick={() => setCurrentPage('search')}
              className={`px-3 md:px-4 py-1 rounded-full text-xs md:text-sm transition-colors ${currentPage === 'search' ? 'bg-white text-emerald-600' : 'bg-white/20 hover:bg-white/30'}`}
            >
              Search
            </button>
            <button 
              onClick={() => setCurrentPage('detail')}
              className={`px-3 md:px-4 py-1 rounded-full text-xs md:text-sm transition-colors ${currentPage === 'detail' ? 'bg-white text-emerald-600' : 'bg-white/20 hover:bg-white/30'}`}
            >
              Detail
            </button>
            <button 
              onClick={() => setCurrentPage('booking')}
              className={`px-3 md:px-4 py-1 rounded-full text-xs md:text-sm transition-colors ${currentPage === 'booking' ? 'bg-white text-emerald-600' : 'bg-white/20 hover:bg-white/30'}`}
            >
              Booking
            </button>
            <button 
              onClick={() => setCurrentPage('dashboard')}
              className={`px-3 md:px-4 py-1 rounded-full text-xs md:text-sm transition-colors ${currentPage === 'dashboard' ? 'bg-white text-emerald-600' : 'bg-white/20 hover:bg-white/30'}`}
            >
              Dashboard
            </button>
          </div>
      ...
      <section className="section-padding bg-white">
        <div className="container-custom">
          <div className="flex items-end justify-between mb-8">
            ...
            <a href="#" className="hidden md:flex items-center gap-2 text-emerald-600 hover:text-emerald-700 font-medium transition-colors">
              View all
          ...
          <div className="mt-6 text-center md:hidden">
            <a href="#" className="inline-flex items-center gap-2 text-emerald-600 hover:text-emerald-700 font-medium transition-colors">
